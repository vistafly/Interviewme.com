...
function buildMockResponse() {
  return {
    ...
    content: [
      {
        type: 'text',
        text: JSON.stringify({ questions: MOCK_QUESTIONS })
      }
    ],
    model: 'mock-mode',
...
app.post('/api/interview', async (req, res) => {
  if (MOCK_MODE) {
    console.log('[MOCK MODE] Returning mock interview questions (no API call)');
    // Simulate a small network delay so the progress bar still animates
...
app.listen(PORT, () => {
  ...
  if (MOCK_MODE) {
    console.log('>>> MOCK MODE ON — returning fake questions, no API calls <<<');
  }
});
