...
// Detect software GPU renderer once at module level
let _isSoftwareGPU = null;
function isSoftwareRenderer() {
  if (_isSoftwareGPU !== null) return _isSoftwareGPU;
  try {
    const c = document.createElement('canvas');
    const gl = c.getContext('webgl') || c.getContext('experimental-webgl');
    if (!gl) { _isSoftwareGPU = true; return true; }
    const info = gl.getParameter(gl.RENDERER) || '';
    gl.getExtension('WEBGL_lose_context')?.loseContext();
    _isSoftwareGPU = /swiftshader|llvmpipe|software|webkit webgl/i.test(info);
  } catch (_) { _isSoftwareGPU = false; }
  return _isSoftwareGPU;
}
...
function GradualBlur(props) {
  ...
  const blurDivs = useMemo(() => {
    const divs = [];
    const effectiveDivCount = isSoftwareRenderer() ? Math.min(config.divCount, 4) : config.divCount;
